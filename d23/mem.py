...
def dfs(curDist=0):
    ...
    key = (tuple(tuple(x) for x in data), tuple(buf))
    if key in searched:
        return searched[key]
    done = True
    for i in range(4):
        ...
        if buf[i] != 0:
            ...
            if bufgood:
                if all(data[dest][j] == 0 or data[dest][j] == curval for j in range(depth)):
                    ...
                    val = dfs(curDist + curval * (1 + cell + abs(dest * 2 + 2 - i)))
                    data[dest][cell] = 0
                    buf[i] = curval

                    searched[key] = val + curval * (1 + cell + abs(dest * 2 + 2 - i))
                    return searched[key]
    ...
    for i in range(4):
        ...
        while curcoord >= 0 and buf[curcoord] == 0:
            if bufok[curcoord]:
                buf[curcoord] = curval
                best = min(best, curval * moved + dfs(curDist + curval * moved))
                buf[curcoord] = 0
            curcoord -= 1
            moved += 1
        curcoord = i * 2 + 2
