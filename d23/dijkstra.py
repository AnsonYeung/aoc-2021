...
def push(curDist, data, buf):
    ...
    # print(curDist)
    heappush(minQ, (curDist, count, deepcopy(data), copy(buf)))
    count += 1
push(0, data, buf)
while True:
    curDist, _, data, buf = heappop(minQ)
        ...
        if buf[i] != 0:
            ...
            if bufgood:
                if all(data[dest][j] == 0 or data[dest][j] == curval for j in range(depth)):
                    ...
                    push(curDist + curval * (1 + cell + abs(dest * 2 + 2 - i)), data, buf)
                    data[dest][cell] = 0
                    buf[i] = curval
                    moved = True
                    break
    ...
    for i in range(4):
        ...
        while curcoord >= 0 and buf[curcoord] == 0:
            if bufok[curcoord]:
                buf[curcoord] = curval
                push(curDist + curval * moved, data, buf)
                buf[curcoord] = 0
            curcoord -= 1
            moved += 1
        curcoord = i * 2 + 2
